...
unsigned char checkFrameByStream(const unsigned char* pFrameToCheck)
{
	...
	else
	{
		...
		return(crc8_frameBased(pFrameToCheck, upperBound+1+M3S_OVERHEAD_LENGTH, M3S_CRC_INITVAL));
	}
}
...
unsigned char m3s_parseToFrame(m3sFrame_t* rParsedFrame, unsigned char* rToParse)
{
